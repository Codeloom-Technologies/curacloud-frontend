...
export const createAppointments = async (payload: any) => {
  ...
  if (!response) {
    ...
    throw new Error(error.message || "Failed to create appointments ");
  }
  return response;
};
...
export const fetchAppointments = async (
  ...
  const response = await apiClient(`/appointments?${queryParams}`);
